...
function page_metabox_content( $post ) {
	...
	array_push( $page_fields, tn_new_meta( 'title=title1&name=name1&size=100%&type=checkbox' ) );
	array_push( $page_fields, tn_new_meta( 'title=title2&name=name2&size=50%&type=text' ) );
	array_push( $page_fields, tn_new_meta( array( 'title' => 'title3', 'type' => 'text' ) ) );
	array_push( $page_fields, tn_new_meta( array( 'title' => 'title4', 'type' => 'textarea', 'size' => array( '100px', '100px' ) ) ) );

	$options = array(
		array( 'label' => 'aaa', 'value' => '1' ),
		array( 'label' => 'bbb', 'value' => '2' )
	);
	array_push( $page_fields, tn_new_meta( array( 'title' => 'title5', 'type' => 'select', 'options' => $options ) ) );
	array_push( $page_fields, tn_new_meta( array( 'title' => 'title6', 'type' => 'color-picker' ) ) );
	array_push( $page_fields, tn_new_meta( array( 'title' => 'title7', 'type' => 'wp-editor' ) ) );
